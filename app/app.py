...
# FIXED: debug=False ensures the Werkzeug debugger is disabled for security.
def init_db():
...
def ctx():
    return {
        ...
        'version':     os.environ.get('APP_VERSION', '1.0.0-secure'),
...
def search():
    ...
    if q:
        # SECURE: Input is passed as a parameter, not concatenated into the string.
        query = "SELECT * FROM users WHERE username = ?"
        try:
            results = DB.execute(query, (q,)).fetchall()
        except Exception as e:
            error = str(e)
    return render_template('search.html', q=q, query=f"SELECT * FROM users WHERE username = '{q}'", results=results, error=error, **ctx())
